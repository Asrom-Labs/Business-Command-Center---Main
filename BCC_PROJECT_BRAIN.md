...
**Status:** WAVE 5 (REPAIR & GUARD) COMPLETE — BUG-11 (channel enums) + BUG-12 (stock fields) FIXED; frontend role guards mirror backend on all 13 built pages; logout/401 now clear bcc_org; stock-changing mutations invalidate ['stock']+['dashboard']. ⚠ ACTION REQUIRED: run migration 005 on Railway (adds walk_in/phone to sales_orders.channel CHECK) — until then, walk_in/phone orders fail the DB constraint. 13/22 pages built; 9 Wave 4 placeholders remain (W7). See Section 14.
...
| ID | Priority | Issue |
|----|----------|-------|
| BUG-11 | ✅ FIXED (W5) | SO channel enum mismatch. Fixed: validators + schema CHECK + translations expanded to canonical set; controller default →walk_in. ⚠ migration 005 must be run on Railway to alter the live DB CHECK. See Section 7. |
...
**W5 status:** BUG-11, BUG-12, N-1, N-2, N-3 all FIXED (15 Jul 2026). Remaining from the audit: N-4 (npm audit), N-5..N-12 (LOW cleanup) still open. ⚠ Migration 005 (channel CHECK) must be run on Railway.
...
| 15 Jul 2026 | Claude Code | v4.9 | WAVE 5 — REPAIR & GUARD. BUG-11 fixed: sales_orders.channel canonical set [walk_in, phone, in_store, whatsapp, instagram, snapchat, tiktok, online, other] — expanded both express validators + schema.sql CHECK; controller default in_store→walk_in; added migration 005 (run on Railway to alter live CHECK — pending); added in_store/snapchat/tiktok translations EN+AR (frontend options were already canonical). BUG-12 fixed: StockPage reads stock_on_hand + derives low-stock from low_stock_threshold client-side. Role guards: action buttons on all 13 built pages gated via useAuth (isAdmin/isStaff, hidden not disabled) to mirror backend route levels; Sidebar Reports readonly→staff; readonly still sees all list/detail views. Logout cleanup: authStore.logout() clears org store; 401 interceptor removes bcc_org. Stock/dashboard invalidations added to PO receive, transfer confirm, SO create/cancel/status/payment/return, returns create. RULE-36 added. Build: 3,048 modules, 0 errors (unchanged — no new modules). DB DDL NOT executed (DATABASE_URL points to Railway prod — migration handed to user). |
